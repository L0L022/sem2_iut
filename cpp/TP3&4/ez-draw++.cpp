 ...
 : win(None),current_event(NULL),_isVisible(true)
{
 ...
 ezDraw->windows.insert(std::pair<Ez_window,EZWindow*>(win,this)); // remplace : windows[win]=this;
}

EZWindow::~EZWindow()
{
 ezDraw->windows.erase(win);
...
{
 EZWindow *pwin = ezDraw->recoverEZWindow(ev->win);
