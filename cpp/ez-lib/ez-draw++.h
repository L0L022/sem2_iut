...
class EZDraw {
 std::map<Ez_window,EZWindow*> windows;
 friend      EZWindow::EZWindow(int w,int h,const char *name); // Permet au constructeur de EZWindow d'enregistrer this dans le map ci-dessus.
 friend      EZWindow::~EZWindow(); // Idem, mais pour supprimer du map ci-dessus.
public:
                    EZDraw();
 virtual            ~EZDraw();
